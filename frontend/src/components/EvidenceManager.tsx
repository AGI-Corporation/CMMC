      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200 text-sm">
            {loading ? (
              <tr><td colSpan={5} className="px-6 py-8 text-center text-gray-500">Loading evidence...</td></tr>
            ) : evidence.length === 0 ? (
              <tr><td colSpan={5} className="px-6 py-8 text-center text-gray-500">No evidence recorded yet.</td></tr>
            ) : evidence.map((ev) => (
              <tr key={ev.id} className="hover:bg-gray-50">
                <td className="px-6 py-4 font-semibold text-gray-900">{ev.source_system}</td>
                <td className="px-6 py-4 text-blue-700 font-bold">{ev.control_id}</td>
                <td className="px-6 py-4">
                  <span className="bg-blue-50 text-blue-700 px-2 py-1 rounded text-xs font-medium">{ev.zt_pillar}</span>
                </td>
                <td className="px-6 py-4 text-gray-500">{new Date(ev.timestamp).toLocaleString()}</td>
