...
**Vulnerability:** HTTPException error responses exposed raw exception details (e.g., `str(e)`), which could leak sensitive internal information to clients.
**Learning:** FastAPIs `HTTPException` detail field should not contain un-sanitized or unhandled error messages, as they can reveal system architecture or sensitive data.
**Prevention:** Always log the raw error internally using `logging.error()` and return a generic, secure error message (e.g., "An internal error occurred") to the client.
