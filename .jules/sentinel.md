...
## 2026-04-11 - Information Leakage via Error Messages
**Vulnerability:** Unhandled exceptions and explicit `HTTPException(detail=str(e))` calls leaked sensitive internal state (e.g., database connection details, API keys) to clients.
**Learning:** FastAPI's default error handling can be too verbose in production. A global exception handler for the base `Exception` class is necessary to catch unplanned failures and return a safe, generic "Internal server error" message. However, the handler MUST explicitly allow `HTTPException` and `RequestValidationError` to pass through to preserve standard RESTful error behaviors (404, 422).
**Prevention:** Always implement a global catch-all exception handler and avoid using `str(e)` in user-facing error messages. Use central logging to capture actual error details for internal review.
