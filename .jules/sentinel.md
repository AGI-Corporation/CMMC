...
## 2026-05-20 - [Information Exposure] Exception Handling in FastAPI Routes
**Vulnerability:** FastAPI routes in `agents/mistral_agent/agent.py` caught exceptions and returned the raw error details (using `str(e)`) to the client via `HTTPException`.
**Learning:** Returning raw exception details in HTTP responses can expose sensitive internal information (stack traces, database details, API key errors) to external clients, which is an Information Exposure vulnerability (CWE-209).
**Prevention:** Always log exceptions internally (e.g., `logging.error(..., exc_info=True)`) and return generic error messages (e.g., "Internal server error") to the external client.
