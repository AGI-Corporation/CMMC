...
## 2025-05-15 - Global Exception Masking
**Vulnerability:** Information disclosure via verbose error messages. Explicitly returning `str(e)` in `HTTPException` detail fields leaked implementation details like stack traces, internal IDs, and potentially secrets if an exception occurred during a sensitive operation.
**Learning:** Catch-all `try...except` blocks in individual routes are brittle and often lead to insecure error disclosure. A global exception handler in FastAPI provides a centralized, secure-by-default way to mask all 500+ errors while still allowing 4xx client errors to pass through.
**Prevention:** Avoid `detail=str(e)` in `HTTPException` constructors. Implement global exception handlers for `Exception` and `StarletteHTTPException` to catch and mask internal server errors before they reach the client.
