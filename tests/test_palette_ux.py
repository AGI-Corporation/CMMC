...
async def test_ssp_zt_pillar_alignment():
    async with AsyncClient(
        ...
        # Check for specific pillars and their progress bars
        assert "| User | AC, IA, PS |" in content
        assert "| Automation & Orchestration | IR, SI, CA |" in content

        # Since we added 2 controls in AC (User pillar) and 1 is implemented, 1 is partial
        # The exact percentage depends on the number of controls in the seeded database.
        # We just want to ensure it's calculated and displayed as a percentage.
        import re

        assert re.search(r"\| User \| .* \| `.*` \d+\.\d+%", content)
