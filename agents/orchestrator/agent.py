...
class ComplianceOrchestrator:
    ...
    async def compute_sprs_score(self, db: AsyncSession) -> Dict[str, Any]:
        ...
        # Performance optimization: Fetch only ID and score_value for SPRS calculation
        ...
        # Optimization: Fetch only status from latest assessments via shared helper
    ...
    async def compute_zt_scorecard(self, db: AsyncSession) -> List[Dict[str, Any]]:
        ...
        scorecard = []
        # Optimization: Fetch only required fields for ZT scorecard
        assessments_map = await get_latest_assessments(
            db,
            columns=[
        ...
        for pillar, domains in self.ZT_DOMAIN_MAP.items():
            # Optimization: Selective columns for ControlRecord
            query = select(ControlRecord.id, ControlRecord.title).where(
