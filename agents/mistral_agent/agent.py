...
async def gap_analysis(req: GapAnalysisRequest, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        logging.error("Failed to perform gap analysis", exc_info=True)
        raise HTTPException(
...
async def code_review(req: CodeReviewRequest, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        logging.error("Failed to perform code review", exc_info=True)
        raise HTTPException(
            status_code=500, detail="An internal server error occurred."
        )
...
async def ask_question(req: QuestionRequest):
    ...
    try:
        ...
        logging.error("Failed to answer question", exc_info=True)
        raise HTTPException(
            status_code=500, detail="An internal server error occurred."
        )
