...
async def gap_analysis(req: GapAnalysisRequest, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        logger.exception(f"Error during gap analysis for {req.control_id}")
        raise HTTPException(
            status_code=500,
            detail="An internal server error occurred during gap analysis.",
        )
...
async def code_review(req: CodeReviewRequest, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="An internal server error occurred during code review.",
        )
...
async def ask_question(req: QuestionRequest):
    ...
    try:
        ...
        logger.exception("Error answering compliance question")
        raise HTTPException(
            status_code=500,
            detail="An internal server error occurred while answering the compliance question.",
        )
