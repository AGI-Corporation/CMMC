...
async def gap_analysis(req: GapAnalysisRequest, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        logging.error("Error in gap_analysis endpoint", exc_info=True)
...
async def code_review(req: CodeReviewRequest, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        logging.error("Error in code_review endpoint", exc_info=True)
        raise HTTPException(status_code=500, detail="An internal server error occurred.")
...
async def ask_question(req: QuestionRequest):
    ...
    try:
        ...
        logging.error("Error in ask_question endpoint", exc_info=True)
        raise HTTPException(status_code=500, detail="An internal server error occurred.")
