...
async def gap_analysis(req: GapAnalysisRequest, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        logging.error("Error in gap analysis", exc_info=True)
        raise HTTPException(
            status_code=500, detail="An internal error occurred during gap analysis."
        )
...
async def code_review(req: CodeReviewRequest, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        logging.error("Error in code review", exc_info=True)
        raise HTTPException(
            status_code=500, detail="An internal error occurred during code review."
        )
...
async def ask_question(req: QuestionRequest):
    ...
    try:
        ...
        logging.error("Error in ask question", exc_info=True)
        raise HTTPException(
            status_code=500,
