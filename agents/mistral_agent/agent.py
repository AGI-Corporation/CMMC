...
async def gap_analysis(req: GapAnalysisRequest, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        logging.error(f"Mistral gap analysis error: {e}", exc_info=True)
...
async def code_review(req: CodeReviewRequest, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        logging.error(f"Mistral code review error: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Internal server error")
...
async def ask_question(req: QuestionRequest):
    ...
    try:
        ...
        logging.error(f"Mistral question error: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Internal server error")
