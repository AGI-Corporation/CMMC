...
async def gap_analysis(req: GapAnalysisRequest, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        logging.error(f"Internal error in gap_analysis: {e}", exc_info=True)
        raise HTTPException(
...
async def code_review(req: CodeReviewRequest, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        logging.error(f"Internal error in code_review: {e}", exc_info=True)
        raise HTTPException(
            status_code=500, detail="An internal server error occurred."
        )
...
async def ask_question(req: QuestionRequest):
    ...
    try:
        ...
        logging.error(f"Internal error in ask_question: {e}", exc_info=True)
        raise HTTPException(
            status_code=500, detail="An internal server error occurred."
        )
