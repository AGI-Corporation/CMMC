...
async def get_compliance_dashboard(db: AsyncSession = Depends(get_db)):
    # Performance Optimization: Select only required columns to avoid ORM overhead of large fields
...
async def calculate_sprs_score(db: AsyncSession = Depends(get_db)):
    # Performance Optimization: Select only required columns (id is used to check against deductions)
