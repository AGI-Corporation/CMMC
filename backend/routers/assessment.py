...
async def get_compliance_dashboard(db: AsyncSession = Depends(get_db)):
    # Performance Optimization: Select only required columns for summary calculations
    # score_value is required for SPRS calculation within this endpoint
    ...
    # Optimization: Selective column fetching for assessments
...
async def calculate_sprs_score(db: AsyncSession = Depends(get_db)):
    # Performance Optimization: Fetch only ID and weight (score_value) for scoring logic
