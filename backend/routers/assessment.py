...
async def get_compliance_dashboard(db: AsyncSession = Depends(get_db)):
    # Optimization: Selective column fetching for ControlRecord
...
@router.get(
    ...
    description=(
        "Calculate the DoD Supplier Performance Risk System (SPRS) score "
        "based on current control implementation status. Score ranges from -203 to 110."
    ),
)
