...
async def get_compliance_dashboard(db: AsyncSession = Depends(get_db)):
    # Optimized: Fetch only necessary columns for dashboard summary
    ...
    # Optimized: Fetch only necessary columns for assessments
...
async def calculate_sprs_score(db: AsyncSession = Depends(get_db)):
    # Optimized: Fetch only necessary columns for SPRS calculation
    # Included id and score_value to ensure deductions are calculated correctly
