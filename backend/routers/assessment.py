...
async def get_compliance_dashboard(db: AsyncSession = Depends(get_db)):
    # Optimized: Selective column fetching to avoid loading large Text fields (description)
    result = await db.execute(
        select(ControlRecord.id, ControlRecord.domain, ControlRecord.level)
    )
    controls = result.all()

    # Optimized: Selective column fetching for assessments
...
@router.get(
    ...
    description=(
        "Calculate the DoD Supplier Performance Risk System (SPRS) score based "
        "on current control implementation status. Score ranges from -203 to 110."
    ),
)
async def calculate_sprs_score(db: AsyncSession = Depends(get_db)):
    # Optimized: Selective column fetching for controls
