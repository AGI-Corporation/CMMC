...
async def get_compliance_dashboard(db: AsyncSession = Depends(get_db)):
    # Performance optimization: Select only necessary columns to avoid loading large description/discussion text
    ...
    # Optimization: Directly fetch only control_id and status for latest assessments via helper
...
async def calculate_sprs_score(db: AsyncSession = Depends(get_db)):
    # Performance optimization: Select only necessary columns (including score_value for deduction logic)
