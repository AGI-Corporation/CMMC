...
# Mapping of CMMC Domains to Zero Trust Pillars for reporting
ZT_PILLAR_DOMAINS = {
    "User": ["AC", "IA", "PS"],
    "Device": ["CM", "MA", "PE"],
    "Network": ["SC", "AC"],
...
async def generate_ssp(
    ...
    for pillar, domains in ZT_PILLAR_DOMAINS.items():
        p_controls = [c for c in controls.values() if c.domain in domains]
        p_total = len(p_controls)
        p_imp = 0
        p_part = 0
        ...
        if p_total > 0:
            p_pct = (p_imp / p_total * 100)
            p_status = get_progress_bar(p_pct)
        else:
            p_status = "⚪ N/A"

        pillar_rows.append(f"| {pillar} | {', '.join(domains)} | {p_status} |")
...
*Showing {len(assessments[:20])} of {len(assessments)} findings.*
