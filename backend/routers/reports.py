...
# Centralized Zero Trust Pillar Mapping
ZT_PILLAR_DOMAINS = {
    "User": ["AC", "IA", "PS"],
    "Device": ["CM", "MA", "PE"],
    "Network": ["SC", "AC"],
...
def get_maturity_pct(assessments: List[AssessmentRecord], domains: List[str]) -> float:
    """Calculate maturity percentage for a set of domains (0-100)."""
    relevant = [
        a
...
async def get_dashboard(
    ...
    return {
        ...
        "zt_pillars": [
            {"pillar": p, "domains": d, "progress": get_maturity_pct(assessments, d)}
