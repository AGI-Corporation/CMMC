...
async def generate_poam(
    ...
    # SECURITY: Sanitize system_name to prevent HTTP Header Injection and Path Traversal
    safe_system_name = re.sub(r"[^a-zA-Z0-9_\-]", "_", system_name)

    return PlainTextResponse(
        content=csv_content,
        media_type="text/csv",
        headers={
            "Content-Disposition": f'attachment; filename="poam_{safe_system_name}.csv"'
        },
    )
