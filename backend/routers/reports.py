...
# Centralized Zero Trust Pillar to CMMC Domain mapping
ZT_PILLAR_DOMAINS = {
    "User": ["AC", "IA", "PS"],
    "Device": ["CM", "MA", "PE"],
    "Network": ["SC", "AC"],
...
def get_maturity_pct(assessments: List[AssessmentRecord], domains: List[str]) -> float:
    ...
    relevant = [a for a in assessments if any(a.control_id.startswith(d + ".") for d in domains)]
    if not relevant:
        return 0.0
    implemented = sum(1 for a in relevant if a.status == "implemented")
    partial = sum(1 for a in relevant if a.status in ["partial", "partially_implemented"])
    return (implemented + partial * 0.5) / len(relevant) * 100
...
"""
    ...
    for pillar, domains in ZT_PILLAR_DOMAINS.items():
        pillar_pct = get_maturity_pct(assessments, domains)
        ssp += f"| {pillar} | {', '.join(domains)} | {get_progress_bar(pillar_pct)} |\n"
...
*Note: Showing first {min(20, len(assessments))} of {len(assessments)} assessment findings.*
