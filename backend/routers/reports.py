...
async def generate_ssp(
    ...
    """
    # Fetch latest assessments with only required columns for SSP
...
async def generate_poam(
    ...
    """
    # Fetch latest assessments with only required columns for POAM
    assessments_dict = await get_latest_assessments(
        db,
        columns=[
            AssessmentRecord.control_id,
...
async def get_dashboard(
    ...
    # Optimization: Only fetch columns needed for dashboard report summary
    assessments_dict = await get_latest_assessments(
