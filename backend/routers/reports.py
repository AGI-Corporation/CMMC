...
async def generate_ssp(
    ...
    """
    # Fetch latest assessments with only required columns for SSP summary and initial findings
    ...
    # Performance: Select only columns needed for SSP report
    controls_result = await db.execute(
        select(ControlRecord.id, ControlRecord.title, ControlRecord.description)
    )
...
async def generate_poam(
    ...
    """
    # Performance: Only fetch columns needed for POA&M
    assessments_dict = await get_latest_assessments(
        db,
        columns=[
            AssessmentRecord.control_id,
...
async def get_dashboard(
    ...
    # Performance: Only fetch status for dashboard counts
    assessments_dict = await get_latest_assessments(
