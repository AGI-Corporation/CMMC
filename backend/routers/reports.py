...
def get_maturity_pct(domains: List[str], assessments: Dict[str, Any]) -> float:
    """
    Calculate maturity percentage for a set of domains.
    - implemented = 1.0
    - partial / partially_implemented = 0.5
    - na = exclude from calculation
    - others = 0.0
    """
    relevant_assessments = [
        a
        for ctrl_id, a in assessments.items()
        if any(ctrl_id.startswith(d + ".") for d in domains)
    ]
    # Filter out N/A
    scored_assessments = [a for a in relevant_assessments if a.status != "na"]
    ...
    for a in scored_assessments:
        if a.status == "implemented":
            total_score += 1.0
        elif a.status in ["partial", "partially_implemented"]:
            total_score += 0.5

    return round((total_score / len(scored_assessments)) * 100, 1)
...
"""
    ...
    for pillar, domains in ZT_PILLAR_DOMAINS.items():
        maturity = get_maturity_pct(domains, assessments_dict)
        ssp += (
...
---
*Generated by AGI Corporation CMMC Compliance Platform v1.0*
"""
        ...
        "zt_pillars": [
            {
                "pillar": pillar,
                "domains": domains,
                "maturity_score": get_maturity_pct(domains, assessments_dict),
            }
            for pillar, domains in ZT_PILLAR_DOMAINS.items()
        ],
        "agents": [
