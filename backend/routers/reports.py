...
# Centralized mapping of ZT Pillars to CMMC domains
ZT_PILLAR_DOMAINS = {
    "User": ["AC", "IA", "PS"],
    "Device": ["CM", "MA", "PE"],
    "Network": ["SC", "AC"],
...
def get_maturity_pct(
    assessments_dict: Dict[str, Any], controls_dict: Dict[str, Any], domains: List[str]
) -> float:
    """Calculate maturity percentage for a set of domains, weighted for UX."""
    relevant_controls = [
        c_id for c_id in controls_dict if any(c_id.startswith(d + ".") for d in domains)
    ]
    ...
    score = 0.0
    count = 0
    for c_id in relevant_controls:
        assessment = assessments_dict.get(c_id)
        if assessment:
            if assessment.status == "na":
                continue
            count += 1
            if assessment.status == "implemented":
                score += 1.0
            elif assessment.status in ["partial", "partially_implemented"]:
                score += 0.5
        else:
            count += 1

    return round(score / count * 100, 1) if count > 0 else 0.0
...
"""
    ...
    for pillar, domains in ZT_PILLAR_DOMAINS.items():
        maturity = get_maturity_pct(assessments_dict, controls, domains)
        ...
        "zt_pillars": [
            {
                "pillar": p,
                "domains": d,
                "maturity_score": get_maturity_pct(assessments_dict, controls_dict, d),
            }
