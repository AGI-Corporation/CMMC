...
async def generate_ssp(
    ...
    """
    # Optimization: Use selective column fetching for AssessmentRecord
    ...
    # Optimization: Use selective column fetching for ControlRecord
    # description and title are required for the SSP report content.
    controls_result = await db.execute(select(ControlRecord.id, ControlRecord.title, ControlRecord.description))
...
async def generate_poam(
    ...
    assessments = list(assessments_dict.values())
    # Optimization: Use selective column fetching for ControlRecord
    # title and zt_pillar are used in the POA&M CSV columns.
