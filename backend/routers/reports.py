...
# Centralized mapping of ZT Pillars to CMMC Domain Prefixes
ZT_PILLAR_DOMAINS = {
    "User": ["AC", "IA", "PS"],
    "Device": ["CM", "MA", "PE"],
    "Network": ["SC", "AC"],
...
def get_maturity_pct(assessments: List[AssessmentRecord]) -> float:
    """Calculate weighted maturity (1.0 for implemented, 0.5 for partial)."""
    if not assessments:
        return 0.0
    scores = {
        "implemented": 1.0,
    ...
    total_score = sum(scores.get(a.status, 0.0) for a in assessments)
    return (total_score / len(assessments)) * 100
