...
def get_maturity_pct(assessments: List[AssessmentRecord], domains: List[str]) -> float:
    """Calculate maturity percentage for a set of domains (1.0 for implemented, 0.5 for partial)."""
    # Exclude N/A status from maturity calculations to avoid penalizing compliance score
    relevant = [
        a
    ...
    for a in relevant:
        if a.status == "implemented":
            ...
            scores.append(0.0)
    return (sum(scores) / len(relevant)) * 100
