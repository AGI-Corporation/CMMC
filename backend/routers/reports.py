...
"""
    ...
    for pillar, domains in ZT_PILLAR_DOMAINS.items():
        # Get all controls belonging to these domains
        pillar_controls = [c for c in controls.values() if c.domain in domains]
        if not pillar_controls:
            ssp += f"| {pillar} | {', '.join(domains)} | {get_progress_bar(0)} |\n"
            continue
        ...
        for c in pillar_controls:
            assessment = assessments_dict.get(c.id)
            if assessment:
        ...
        pillar_pct = round(
            (pillar_implemented + 0.5 * pillar_partial) / pillar_total * 100, 1
        )
