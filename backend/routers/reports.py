...
def get_maturity_pct(assessments: List[Any]) -> float:
    """Calculate maturity % giving 0.5 weight to partial implementations."""
    if not assessments:
        return 0.0
    impl = sum(1 for a in assessments if a.status == "implemented")
    part = sum(1 for a in assessments if a.status in ["partial", "partially_implemented"])
    return (impl + (part * 0.5)) / len(assessments) * 100
...
async def generate_ssp(
    ...
    # Dynamic ZT Pillar Progress
    zt_rows = ""
    for pillar, domains in ZT_PILLAR_DOMAINS.items():
        pillar_assessments = [a for a in assessments if a.control_id.split(".")[0] in domains]
        pct = get_maturity_pct(pillar_assessments)
...
*Note: Showing {min(20, len(assessments))} of {len(assessments)} findings.*
