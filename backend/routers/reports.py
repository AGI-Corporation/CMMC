...
async def generate_ssp(
    ...
    # Selective column fetching for ControlRecord
    # Including domain for template alignment
    controls_result = await db.execute(
        select(ControlRecord.id, ControlRecord.title, ControlRecord.domain)
    )
...
async def generate_poam(
    ...
    """
    # Selective column fetching for AssessmentRecord
    assessments_dict = await get_latest_assessments(
        db,
        columns=[
            AssessmentRecord.control_id,
    ...
    # Selective column fetching for ControlRecord
    # Including title and zt_pillar for CSV rows
    controls_result = await db.execute(
