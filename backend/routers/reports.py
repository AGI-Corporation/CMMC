...
async def generate_ssp(
    ...
    for pillar, domains in ZT_PILLAR_DOMAINS.items():
        pillar_assessments = [
            a
            for a in assessments
            if any(a.control_id.startswith(domain + ".") for domain in domains)
        ]
        if pillar_assessments:
            ...
            pillar_pct = 0.0
        pillar_stats.append(
            {"pillar": pillar, "domains": ", ".join(domains), "pct": pillar_pct}
        )
...
| ZT Pillar | CMMC Domains | Alignment Status |
|-----------|--------------|------------------|
"""

    for stat in pillar_stats:
        ssp += f"| {stat['pillar']} | {stat['domains']} | {get_progress_bar(stat['pct'])} |\n"
