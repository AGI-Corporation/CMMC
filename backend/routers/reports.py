...
def get_maturity_pct(assessments: List[AssessmentRecord]) -> float:
    """Calculate weighted maturity percentage (1.0 for implemented, 0.5 for partial)."""
    if not assessments:
        return 0.0
    scores = []
    for a in assessments:
        if a.status == "implemented":
            ...
            scores.append(0.0)
    return (sum(scores) / len(assessments)) * 100
...
async def generate_ssp(
    ...
    # Calculate Zero Trust Pillar Maturity
    zt_rows = []
    for pillar, domains in ZT_PILLAR_DOMAINS.items():
        pillar_assessments = [
            a for a in assessments if any(a.control_id.startswith(d) for d in domains)
        ]
...
"""
    ...
    # Count total findings for the summary line
    total_findings = len(assessments)
    ssp += f"*Showing {min(20, total_findings)} of {total_findings} findings.*\n\n"
