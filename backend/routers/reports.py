...
def get_maturity_pct(assessments: List[Any], domains: List[str]) -> float:
    """Calculate weighted maturity (1.0 for implemented, 0.5 for partial) for domains."""
    ...
    total_score = sum(scores.get(a.status, 0.0) for a in relevant)
    return (total_score / len(relevant)) * 100
...
*Note: Showing {min(20, len(assessments))} of {len(assessments)} assessment findings for readability.*
