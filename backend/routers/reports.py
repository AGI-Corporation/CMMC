...
async def generate_ssp(
    ...
    for pillar, domains in ZT_PILLAR_DOMAINS.items():
        ...
        if not pillar_assessments:
            ...
            p_total = len(pillar_assessments)
            p_impl = sum(1 for a in pillar_assessments if a.status == "implemented")
            p_pct = (p_impl / p_total) * 100
            status = get_progress_bar(p_pct, width=5)

        pillar_rows += f"| {pillar} | {', '.join(domains)} | {status} |\n"
