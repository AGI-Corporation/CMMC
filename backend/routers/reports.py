...
async def generate_ssp(
    ...
    """
    # BOLT OPTIMIZATION: Use selective column fetching for assessments and controls
...
async def generate_poam(
    ...
    """
    # BOLT OPTIMIZATION: Use selective column fetching to minimize memory footprint
    assessments_dict = await get_latest_assessments(
        db,
        columns=[
            AssessmentRecord.control_id,
...
async def get_dashboard(
    ...
    # BOLT OPTIMIZATION: Fetch only 'status' to compute stats efficiently
    assessments_dict = await get_latest_assessments(
