...
# Centralized mapping for Zero Trust pillars to CMMC domains
ZT_PILLAR_DOMAINS = {
    "User": ["AC", "IA", "PS"],
    "Device": ["CM", "MA", "PE"],
    "Network": ["SC", "AC"],
...
async def generate_ssp(
    ...
    # Calculate Zero Trust Pillar compliance
    zt_pillar_rows = ""
    for pillar, domains in ZT_PILLAR_DOMAINS.items():
        pillar_controls = [c for c in controls.values() if c.domain in domains]
        total_p = len(pillar_controls)
        implemented_p = sum(
            1
            for c in pillar_controls
            if assessments_dict.get(c.id)
            and assessments_dict[c.id].status == "implemented"
        )
        pct = (implemented_p / total_p * 100) if total_p > 0 else 0
        zt_pillar_rows += (
            f"| {pillar} | {', '.join(domains)} | {get_progress_bar(pct)} |\n"
        )
