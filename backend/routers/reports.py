...
from backend.db.database import get_db, AssessmentRecord, ControlRecord, EvidenceRecord, get_latest_assessments as db_get_latest_assessments

router = APIRouter()

async def get_latest_assessments(db: AsyncSession):
    """Shim for legacy usage in this router, using the centralized helper."""
    assessments_map = await db_get_latest_assessments(db)
    return list(assessments_map.values())
