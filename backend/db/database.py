...
async def get_latest_assessments(db: AsyncSession) -> Dict[str, AssessmentRecord]:
    """
    Fetch the most recent assessment for each control.
    Optimized via the idx_control_date composite index.
    """
    sub_q = (
        select(
