...
async def get_latest_assessments(
    ...
    Supports selective column fetching to avoid loading large text fields.
    """
    sub_q = select(
    ...
    if columns:
        # Ensure control_id is always fetched for the return dictionary mapping
    ...
    if columns:
        # Return dictionary mapping control_id to Row object
        return {r.control_id: r for r in result.all()}
    else:
        # Return dictionary mapping control_id to full Model instance
