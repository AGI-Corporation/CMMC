...
async def get_latest_assessments(db: AsyncSession, control_ids: list[str] = None):
    ...
    Optimized via idx_control_date index and optional control_id filtering.
    """
    # Use a subquery to find the max assessment_date per control_id
    sub_q = (
        select(
    ...
    # Join back to AssessmentRecord to get the full record for each latest date
    assessments_q = (
    ...
    result = await db.execute(assessments_q)
