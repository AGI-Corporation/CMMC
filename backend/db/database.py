...
async def get_latest_assessments(
    ...
    Optionally fetch only specific columns to reduce I/O and ORM overhead.
    """
    sub_q = select(
    ...
    if columns:
        # Ensure control_id is present if columns are specified
        cols = list(columns)
    ...
    if columns:
        # Returns SQLAlchemy Row objects for selective fetching
