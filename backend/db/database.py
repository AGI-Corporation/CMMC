...
async def get_latest_assessments(
    ...
    Optionally selective column fetching for even better performance.
    """
    sub_q = select(
    ...
    if columns:
        # Ensure control_id is always included for mapping
        actual_cols = list(columns)
        if AssessmentRecord.control_id not in actual_cols:
            actual_cols.append(AssessmentRecord.control_id)
        query = select(*actual_cols)
    else:
    ...
    if columns:
        # Returns Row objects which support attribute access like a.control_id
