...
async def get_latest_assessments(
    ...
    If columns are provided, fetches only those columns and returns a dict of Row objects.
    """
    sub_q = select(
    ...
    if columns:
        # Ensure control_id is present for the dictionary mapping
        if AssessmentRecord.control_id not in columns:
            query_cols = columns + [AssessmentRecord.control_id]
        else:
            query_cols = columns
