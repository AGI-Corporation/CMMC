...
async def get_latest_assessments(
    ...
    Optionally filtered by a list of control_ids and specific columns.
    """
    sub_q = select(
    ...
    if columns:
        # Create a copy to avoid in-place mutation of the input list
        selected_cols = list(columns)
        # Ensure control_id is always included for the dictionary mapping
        if AssessmentRecord.control_id not in selected_cols:
            selected_cols.append(AssessmentRecord.control_id)
    ...
    if columns:
        # result.all() returns Row objects which allow attribute access (row.status)
