...
async def get_latest_assessments(
    ...
    If columns are provided, only those columns are fetched (returning Row objects).
    """
    sub_q = select(
    ...
    if columns:
        # Clone list to avoid side effects
        cols = list(columns)
        # Ensure control_id is present for mapping
    ...
    # Return dict of full Model instances
