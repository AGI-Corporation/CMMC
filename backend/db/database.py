...
async def get_latest_assessments(
    ...
    Optionally select specific columns to avoid loading heavy fields.
    """
    sub_q = select(
    ...
    if columns:
        # Ensure control_id is included for the mapping dictionary
        # Use a new list to avoid in-place modification of the caller's list
        query_cols = (
            columns
            if AssessmentRecord.control_id in columns
            else list(columns) + [AssessmentRecord.control_id]
        )
    ...
    if columns:
        # When specific columns are requested, we return Row objects
        return {a.control_id: a for a in result.all()}
    else:
        # Return full model instances
