...
async def get_latest_assessments(
    ...
    Optionally filtered by a list of control_ids or specific columns for better performance.
    """
    sub_q = select(
    ...
    # If specific columns are requested, use them; otherwise fetch the full model
    if columns:
        # Ensure control_id is included for mapping
        if AssessmentRecord.control_id not in columns:
    ...
    if columns:
        # When fetching specific columns, result.all() returns Row objects
        rows = result.all()
