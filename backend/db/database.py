...
async def get_latest_assessments(
    ...
    Optionally specify columns to fetch only necessary fields (e.g. status, score_value).
    """
    sub_q = select(
    ...
    if columns:
        # Optimization: Fetch only specific columns. Ensure control_id is present for mapping.
        if not any(AssessmentRecord.control_id is c for c in columns):
            columns = list(columns) + [AssessmentRecord.control_id]
    ...
    if columns:
        # Returns Row objects which support attribute access (e.g. row.status)
