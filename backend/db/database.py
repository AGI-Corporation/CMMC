...
async def get_latest_assessments(
    ...
    Optionally takes a list of columns for selective fetching.
    """
    sub_q = select(
    ...
    if columns:
        # Clone list and ensure control_id is included for the mapping dictionary
        cols = list(columns)
    ...
    if columns:
        # Return mapping of control_id to Row objects
        return {a.control_id: a for a in result.all()}
    else:
        # Return mapping of control_id to full Model instances
