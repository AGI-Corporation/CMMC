...
class AssessmentRecord(Base):
    ...
    # Bolt: Performance Optimization - Composite index for latest assessment per control lookup
    __table_args__ = (
...
async def get_latest_assessments(db: AsyncSession):
    """
    Bolt: Centralized helper to fetch the latest AssessmentRecord for each control_id.
    Leverages the idx_control_date composite index for performance.
    """
    sub_q = (
        select(
